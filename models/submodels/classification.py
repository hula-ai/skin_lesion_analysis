...
class DropConnect(Wrapper):
    def __init__(self, layer, prob=0., **kwargs):
        self.prob = prob
        self.layer = layer
    ...
    def call(self, x):
        if 0. < self.prob < 1.:
            ...
            self.layer.kernel = K.dropout(self.layer.kernel, self.prob) * (1 - self.prob)
            # self.layer.bias = K.in_train_phase(K.dropout(self.layer.bias, self.prob), self.layer.bias)
            self.layer.bias = K.dropout(self.layer.bias, self.prob) * (1 - self.prob)
...
def default_classification_model(input_shape=None,
    ...
    if dropout_rate > 0.:
        outputs = keras.layers.Dropout(rate=dropout_rate, name='drop_5')(outputs, training=True)
    ...
    for i in range(num_dense_layers):
        ...
        if dropout_rate > 0.:
            outputs = keras.layers.Dropout(rate=dropout_rate, name='drop_6')(outputs, training=True)
